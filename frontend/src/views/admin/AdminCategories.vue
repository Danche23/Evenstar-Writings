  ...
  <div>
    <div class="admin-page-head flex-between">
      ...
      <div class="head-actions">
        <router-link to="/admin/categories/sort" class="btn btn-outline btn-sm">↕ 排序布局</router-link>
        <button class="btn btn-primary btn-sm" @click="openDialog()">＋ 新建分类</button>
      </div>
    </div>
      ...
      <el-table :data="list" v-loading="loading" style="width: 100%">
        ...
        <el-table-column label="操作" width="140">
          <template #default="{ row }">
            ...
            <el-popconfirm title="删除后该分类关联将清空，确定？" width="220" @confirm="remove(row)">
    ...
    <el-dialog v-model="dialogVisible" :title="editingId ? '编辑分类' : '新建分类'" width="400px">
      <el-form label-position="top" @submit.prevent>
        <el-form-item label="名称">
          <el-input v-model="name" maxlength="50" placeholder="分类名称" @keyup.enter="submit" />
        </el-form-item>
      </el-form>
      <template #footer>
...
import { listCategories, adminCreateCategory, adminUpdateCategory, adminDeleteCategory } from '@/api/category'
...
async function submit() {
  ...
  try {
    if (editingId.value) await adminUpdateCategory(editingId.value, name.value.trim())
    else await adminCreateCategory(name.value.trim())
