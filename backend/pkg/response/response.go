...
import (
	...
	"github.com/Danche23/Evenstar-Writings/pkg/errors"
...
// Error 错误响应（HTTP 状态码由业务码映射，REST 语义码）
func Error(c *gin.Context, code int, message string) {
	c.JSON(errors.HTTPStatus(code), Response{
		Code:    code,
		Message: message,
	})
}
...
func BizError(c *gin.Context, err error) {
	if bizErr, ok := err.(*errors.BizError); ok {
		c.JSON(errors.HTTPStatus(bizErr.Code), Response{
			Code:    bizErr.Code,
...
type PageRequest struct {
	...
	Size int `form:"page_size" binding:"required,min=1,max=100"` // 每页大小，最大100
}
